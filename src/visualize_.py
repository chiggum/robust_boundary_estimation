...
class Visualize:
    ...
    def data(self, X, labels=None, title='', figsize=None, s=5,
        ...
        if X.shape[1] == 2:
            sc = plt.scatter(X[:,0], X[:,1], s=s, c=labels, cmap=cmap, vmin=vmin, vmax=vmax, alpha=alpha)
            if focus_pts is not None:
                sc = plt.scatter(X[focus_pts,0], X[focus_pts,1], s=s_focus_scale*s, c='r')
            if axis_image:
                plt.axis('image')
            if colorbar:
                ax = plt.gca()
                ...
                plt.colorbar(sc, cax=cax, ticks=cbar_ticks, format=ticker.FormatStrFormatter('%.2f'))
                plt.sca(ax)
        elif X.shape[1] == 3:
            ax = fig.add_subplot(projection='3d')
            if azim and elev:
                ax.view_init(azim=azim, elev=elev, roll=roll)
            p = ax.scatter(X[:,0], X[:,1], X[:,2], s=s, c=labels, cmap=cmap, vmin=vmin, vmax=vmax, alpha=alpha)
            if focus_pts is not None:
